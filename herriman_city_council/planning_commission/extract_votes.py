...
def extract_meeting(path, rel_source, date, year, title):
    ...
    for i, a in enumerate(anchors):
        ...
        if out_m:
            ...
            # PC-grammar guard (T3.1(j) sync 2026-07-17): the PC prints WORD-form
            # tallies ("a vote of 4 to 3"), so the council's trailing-page-number
            # strip must NOT eat a tally digit preceded by "to " — only a genuine
            # leaked page number ("... 4 to 3 11" -> "4 to 3"). The council uses
            # colon tallies ("3:2") and never hits this.
            result = re.sub(r"\s+(?<!to )\d{1,3}$", "", result)
