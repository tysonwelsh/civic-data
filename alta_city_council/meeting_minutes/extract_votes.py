...
# --------------------------------------------------------------------------
# SUB-MEETING BODY WALK (2026-07-29)
# --------------------------------------------------------------------------
# An Alta "council minutes" PDF often minutes MORE THAN ONE meeting: the town's
# BUDGET COMMITTEE (Mayor + 2 councilmembers + the staff TREASURER) meets right
# before the Town Council and its minutes are printed in the SAME document, as a
# section opened by "CALL THE BUDGET COMMITTEE MEETING TO ORDER". Hard-coding
# body=Council for every motion in the file therefore recorded Budget Committee
# actions as TOWN COUNCIL votes -- which put Treasurer CRAIG HEIMARK (staff until
# he was certified to the Council for the 2026 term) in the 2022 council roll on
# 2022-04-13 ("VOTE: In favor: Bourke, Morgan, Byrne and Heimark" -- the budget
# committee, not the council), and attributed 3 more BC mover/seconder slots to
# him in 2023. The votes are REAL and stay in the record; only the BODY was wrong.
# This walks the sub-meeting headers and labels each motion with the body that was
# actually sitting (repo pattern: slc RDA/CRA/LBA, holladay LBA, herriman CDRA).
# Default is the dataset's own body, so a file with no sub-meeting is unchanged.
BC_START_RE = re.compile(
    r"^[\s.,|_]*\d{0,2}\s*[.,]?\s*CALL\s+THE\s+BUDGET\s+COMMITTEE\s+MEETING\s+TO\s+ORDER", re.I)
# any OTHER "CALL THE <x> TO ORDER" closes the budget-committee section (the town
# council meeting, a public hearing, the joint BC+council work session, ...), as
# does a standalone "TOWN COUNCIL MEETING" banner line.
OTHER_START_RE = re.compile(
    r"^[\s.,|_]*\d{0,2}\s*[.,]?\s*CALL\s+THE\s+(?!BUDGET\s+COMMITTEE\s+MEETING\b)", re.I)
COUNCIL_BANNER_RE = re.compile(r"^\s*(?:ALTA\s+)?TOWN\s+COUNCIL\s+MEETING\s*$", re.I)

def body_walk(lines):
    """Per-line body label: 'BudgetCommittee' inside a budget-committee sub-meeting,
    otherwise this dataset's BODY. Never guesses -- only explicit headers switch."""
    out, cur = [], BODY
    for ln in lines:
        if BODY == "Council" and BC_START_RE.match(ln):
            cur = "BudgetCommittee"
        elif OTHER_START_RE.match(ln) or COUNCIL_BANNER_RE.match(ln):
            cur = BODY
        out.append(cur)
    return out
...
def parse_meeting(lines):
    ...
    for mno, i in enumerate(motion_idx, 1):
        ...
        votes.append({
            "body": line_body[i], "motion_no": mno, "motion": motion_text,
