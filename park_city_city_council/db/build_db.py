...
CREATE TABLE vote(
  ...
  note TEXT,   -- nullable annotation: 'Mayor tie-break' (parenthetical in the source CSV value)
               -- or 'override: ...' (contradictory source lists resolved via db/vote_overrides.csv)
...
def main():
    ...
    def get_app(app_key, body, nm, title):
        ...
        return app[app_key]
    # ---- vote-value normalization + explicit conflict resolution (2026-07-02) ----
    # The flat all_votes.csv is city-faithful/verbatim: it carries (a) annotated values like
    # "Nay (Mayor tie-break)" and (b) genuine clerk errors where the minutes list a member in
    # BOTH the AYES and NAYS/ABSTAIN lists (two rows, contradictory values). Previously
    # INSERT OR IGNORE silently dropped both classes (11 rows, incl. BOTH mayoral tie-breaks).
    # Now: parenthetical annotations are split into (schema-legal value, note); contradictory
    # (motion, person) pairs MUST have a documented resolution in db/vote_overrides.csv
    # (source_file,motion_no,member,...,resolution) — resolution is a legal vote value, or
    # 'exclude' for truly unresolvable rows. Any conflict NOT covered fails the build loudly.
    VOTE_NOTE_RE = re.compile(r"^(Aye|Nay|Abstain|Recuse|Absent|Excused)\s*\((.+)\)\s*$")
    ...
    if os.path.exists(VOTE_OVERRIDES):
        for r in csv.DictReader(open(VOTE_OVERRIDES)):
            vote_ov[(r["source_file"], r["motion_no"], person_key(norm_person(r["member"])))] = \
                r["resolution"].strip()
    vbym = {}
    for src, mno, nm, vv in votes:
        note = None
        m = VOTE_NOTE_RE.match(vv.strip())
        if m:
            vv, note = m.group(1), m.group(2)
        if vv not in LEGAL_VALUES:
            sys.exit(f"BUILD FAILED: illegal vote value {vv!r} for {nm} on {src} motion {mno}")
        vbym.setdefault((src, mno), []).append((nm, vv, note))
    mid = {}
    for (src, mno), m in motions.items():
        ...
        mvotes = vbym.get((src, mno), [])  # [(name, value, note)]
    ...
    for (src, mno), vs in vbym.items():
        by_person = {}
        for nm, vv, note in vs:
            n_csv_rows += 1
            p = pers(nm)
            if p is None:
                n_no_person += 1; continue
            by_person.setdefault((p, nm), []).append((vv, note))
        for (p, nm), vals in by_person.items():
            distinct = sorted({v for v, _ in vals})
            if len(distinct) == 1:
                vv = distinct[0]
                note = next((nt for _, nt in vals if nt), None)
                ...
                if res not in LEGAL_VALUES:
                    sys.exit(f"BUILD FAILED: vote_overrides.csv resolution {res!r} is not a "
                             f"legal vote value (or 'exclude') for {nm} on {src} motion {mno}")
                vv, note = res, f"override: source lists {'+'.join(distinct)}; resolved per db/vote_overrides.csv"
                n_merged += len(vals) - 1
            cur.execute("INSERT INTO vote(motion_id,person_id,vote_value,note) VALUES(?,?,?,?)",
                        (mid[(src, mno)], p, vv, note))
            n_inserted += 1
    if conflicts_unresolved:
        for src, mno, nm, distinct in conflicts_unresolved:
            print(f"UNRESOLVED VOTE CONFLICT: {src} motion {mno} {nm}: {'+'.join(distinct)}",
                  file=sys.stderr)
        sys.exit(f"BUILD FAILED: {len(conflicts_unresolved)} contradictory (motion, person) vote "
                 f"pair(s) not covered by db/vote_overrides.csv — add documented resolutions "
                 f"(a legal vote value or 'exclude'); rows are never silently dropped.")
    ...
    # vote-row reconciliation: every named CSV row must be accounted for (never silently dropped)
    if n_csv_rows != n_inserted + n_merged + n_excluded + n_no_person:
        problems.append("vote row reconciliation drift")
    ...
    print(f"  vote reconciliation: {n_csv_rows} named CSV rows = {n_inserted} inserted "
          f"+ {n_merged} merged (duplicate/override pairs) + {n_excluded} excluded (override) "
          f"+ {n_no_person} unresolvable person")
    tb = cur.execute("SELECT COUNT(*) FROM vote WHERE note LIKE '%tie-break%'").fetchone()[0]
    print(f"  mayoral tie-break votes preserved (note LIKE '%tie-break%'): {tb}")
