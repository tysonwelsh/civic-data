...
FAIL_TALLY_PREVAILING_FIRST = True
...
KNOWN_TALLY_MISMATCHES = {
    ("2021-03-10", "4"):
        "faithful source contradiction: the roll call records 'Council Chair Jacob - No' "
        "yet the minutes state 'The motion passed 7-0' (6 Yes + 1 No printed); the roll "
        "call is kept verbatim.",
    ("2025-01-28", "1"):
        "faithful source formatting error (RDA consent agenda): the printed roll puts "
        "all six members on the 'NO:' line with 'YES:' empty, then states 'The motion "
        "passed 6-0'; rows keep the printed NO list verbatim.",
    # ("2023-12-20", "7") removed 2026-07-02 (Phase 3.5): the double-adjourn
    # roll/result mis-pairing was fixed in extract_votes.py — m7 is now the
    # first adjourn motion (3-3 Fail, roll verbatim); the second (5-1 Pass,
    # narrative-only) is m8 with names_recorded:false.
}
