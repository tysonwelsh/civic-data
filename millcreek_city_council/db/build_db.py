...
"""
build_db.py — normalized relational database for MILLCREEK CITY's civic vote data
(prose/PDF minutes portal — the vendor-agnostic prose template, adapted for Millcreek).

This is the WITHIN-BODY EXACT CORE. It consumes the per-body denormalized vote CSVs
(meeting_minutes/all_votes.csv [Council + CRA] + planning_commission/all_votes.csv
[PlanningCommission], both the standard 13-col schema) and produces db/millcreek.db
...
Millcreek minutes carry NO structured agenda/matter key, so the project key is RESOLVED
FROM PROSE — EXCEPT the Planning Commission, whose motions cite a Utah planning CASE NUMBER
`<PREFIX>-<YY>-<NNN>` (CU/ZM/ZT/SD/GP/EX/SV/PUD/... e.g. ZM-21-001, CU-17-004). That case
number is an EXACT within-body key and is the top prose-resolution tier here. Council motions
are mostly terse and ordinance/resolution-number-keyed ("adopt Ordinance No. 17-44") and cite
NO case number — EXCEPT a handful (~16 rows) that DO cite the PC case number verbatim, so
unlike South Jordan's one-sided PL bridge, Millcreek's case-number key genuinely (thinly)
bridges PC->Council; that cross-body tie is scored in build_referrals.py.
...
_existing = sorted(glob.glob(os.path.join(HERE, "*.db")))   # reuse an existing db name; else millcreek.db
DB = _existing[0] if _existing else os.path.join(HERE, "millcreek.db")
...
# Utah planning CASE NUMBER (<PREFIX>-<YY>-<NNN>) — the PC's exact within-body case key.
# Prefixes observed in Millcreek PC minutes: CU/CUP (conditional use), ZM (zone map/rezone),
# ZT (zone text), SD/SDA (subdivision), GP (general plan), EX (exception), SV (street vacation),
# PUD (planned unit dev), FC/LB/RC/SP (misc land-use). 3-part (PREFIX-YY-NNN) so it never
# collides with the 2-part Ordinance/Resolution "YY-NN" numbers Council cites.
CASE_RE = re.compile(r'\b(?:CU|CUP|ZM|ZT|SD|SDA|GP|EX|SV|PUD|FC|LB|RC|SP)-\d{2}-\d{2,3}\b', re.I)
