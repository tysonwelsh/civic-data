...
> **2026-08-14 addendum — SLC CF accessibility re-hunt; ⚠ gov.db is now STALE for slc CF.**
> Owner question: is SLC's portal fixed? **No** — `GetElections` still 503 and the body is
> **byte-identical** to the 2026-08-02 retained snapshot bar Cloudflare's rotating email
> token, balance table still stamped "April 2026"; `CandidateReporting` still 500; sibling
> app still 401. **What DID open is the Internet Archive:** the donation interstitial cleared,
> so all 5 previously-blocked objects were recovered — `David_Spatafore` + `J_Michael_Clara`
> (real, born-digital, itemized 2003 filings) and the 3 remaining §2.46.080 notices.
> On-disk slc CF is now **10 filings / 248 contributions / 176 expenditures** (was 8/222/162),
> rebuilt through the existing path, **additive only: 44 insertions, 0 deletions**,
> validate_finance PASS 0/0, geometry spot-checked. 2003 tranche = 10 of 11 (`Dale_Lambert`
> permanent). Also added `raw/declared_candidates/` (2025 + 2021 roster snapshots — spine
> only, no money, not filings) and found **`webdme.slcgov.com`**, an undocumented SLC
> Laserfiche host with 8 live public apps + anonymous full-text search — **no CF subtree
> there** (earned negative) but a real lead for slc minutes/packets (LEADS.md).
> **⚠ ACTION OWED: `scripts/build_cities_db.py` was deliberately NOT run** (repo-level
> coordination rule). gov.db still holds slc cf_filing **8** / cf_contribution **222** /
> cf_expenditure **162** — the flat CSVs are canonical and ahead of it. **Re-federate before
> quoting slc CF from gov.db.** Full probe log: the 2026-08-14 addendum in
> `slc_city_council/campaign_finance/RECON_2026-08-02.md`.
> **GRAMA package IS NOW DRAFTED** — `slc_city_council/campaign_finance/GRAMA_PREP_2026-08-14.md`
> (process, contacts, cost analysis, statutory hooks, 2 ready-to-paste requests). **NOT SENT —
> owner action.** Key research finding: the records are Utah State Archives **Series 26415**,
> SLC City Recorder, 1981–, classification **Public**, retention **PERMANENT** (GRS-282;
> municipal item GRS-1011 = "retain permanently, transfer to archives"), and the Archives
> holds no copy — so the city holds them and "destroyed" is not an available answer. Utah Code
> **§10-3-208(10)** independently obliges the Recorder to post these on the city website within
> 7 business days of filing, which makes the fee-waiver and expedited-response asks strong.
...
## ARCHIVE ANCHOR 2026-08-01-COUNTY-ACQUISITION

### Prior HANDOFF banner (2026-08-01 package spec), verbatim
...
- [x] **st_george 2023 Larkin cycle total sums a SUPERSEDED filing (state-channel amendment
  never ingested) — ✅ CLOSED 2026-08-01, same session it was filed.** Found by the
  washington_county wave agent; VERIFIED at source by the coordinator (direct PDF read of
  `municipal.utah.gov/washington\2024\St. George\2024.04.01  Larkin Amended 08.29.2023
  Form.pdf`). Fix executed: both amended filings ingested (raw + page-marked text sidecars +
  `_fetch_log.jsonl` + index rows with new `filing_type=amended` /
  `source_archive=state_disclosures` values), vision-transcribed by claude-fable-5 with
  itemized sums = stated totals EXACT (Larkin 80 contribs $22,555.00 / 21 expends $11,804.99;
  Kemp 16/$11,300.00, 10/$9,409.28 — unchanged from his original, completeness-only), driver
  supersession markers verified on both originals, cycle_totals re-derived: **Larkin 2023 =
  $33,610 raised / $31,491.92 spent** (was 35,745/34,665.24). `validate_finance.py` PASS.
  Docs corrected same session (city + module CLAUDE.md: 106 rows/16 raws; the falsified
  "disclosures.utah.gov verified empty" claim rewritten). Backups:
  `_backups/2026-08-01-county-acquisition/st_george/`.
...
## ARCHIVE ANCHOR 2026-08-01-HANDOFF — the 2026-07-31→08-01 session banner (verbatim)
...
## ARCHIVE ANCHOR 2026-08-17-PHASEB-RESUME (prior HANDOFF banner, verbatim)

# HANDOFF — resume point (2026-08-14: TRANCHE 3 PHASE B 4-COUNTY WAVE, PAUSED BY OWNER)

> **Read in order: root `CLAUDE.md` → this file → `TODO.md` → `GOTCHAS.md`. Options menu:
> `LEADS.md`. Publish criteria: `SHIP_GATE.md`. One session banner, overwritten per handoff
> (prior banner: TODO_ARCHIVE.md anchor 2026-08-14-PHASEB-PAUSE).**

## Why paused, and the resume trigger

The owner paused the wave DELIBERATELY on 2026-08-14 ~13:15 EDT (weekly usage-limit pacing;
mobile laptop). All agents were stopped cleanly — several finished their in-flight filing
first — and every completed filing is checkpointed on disk. **Resume when the owner says
so (expected ~week of 2026-08-17 when the limit resets).** Nothing is corrupted, nothing
is half-written that the audit path can't catch.

## Wave state (owner-approved 2026-08-14: juab · wasatch · summit · weber, Opus, B2 contract)

- **juab — CLOSED & VERIFIED.** All 27 filings itemized (187 rows: 46C+141E); 34/48 sides
  exact, 3 filer deltas verbatim, 9 no-schedule sides ledgered; validate PASS. Close-out in
  `juab_county/campaign_finance/AVAILABILITY.md` (dated 2026-08-14 section).
- **wasatch — CLOSED & VERIFIED.** 111/111 filings carry an itemized layer (851 rows:
  346C+505E); 168 sides exact, 20 verbatim deltas, 0 withheld; the tableab date-grammar
  family fix landed (+4 tests, 52/52). Close-out in its AVAILABILITY.md.
- **summit — PARTIAL, resumable.** Leg 1 verified 24/116 scans (335 rows; close-out in
  AVAILABILITY.md). Killed/stopped legs 2–3 left ~92 `vision/` caches marked itemized and
  **114 staged record files** in `_backups/2026-08-14-tranche3/summit-b/records/` —
  **ALL UNTRUSTED until re-screened** (see resume brief). Resume kit in that same dir:
  `AGENT_BRIEF.md` (per-row contract + per-filing shapes), `wave_stats.py --residue`
  (derives the live queue), `checkpoint.py` (invariants), `rowbands.py`/`fitgrid.py`
  (measured-geometry tooling; county-agnostic promotion is a filed lead).
- **weber — PARTIAL, resumable, NO verified close-out yet.** ~35 `vision/` caches marked
  itemized + **47 staged records** in `weber_county/campaign_finance/_itemized_records/`;
  working dir `_backups/2026-08-14-weber-cf/` includes an **UNFINISHED `align.py`** (was
  mid-edit at the first kill — treat as unfinished). Check
  `_audits/cf-calibration-suite/runs.md` for a weber-configuration calibration pass; run
  the pre-flight if none is recorded or the configuration changes.

Kill history this wave (checkpoint discipline held through all three): overnight
session-limit kill ~02:05 · laptop-network watchdog kill ~12:05 (all agents froze at once —
that signature = the OWNER's connection dropped, not the API) · deliberate owner pause
~13:15.

## Resume brief (for the next coordinator)

1. **Present the launch for owner approval** (standing rule: count / per-agent model /
   effort), then run ONE Opus resume agent per open county (summit, weber). **Fan-out cap:
   ≤3 concurrent chunk sub-agents per county, declared in the final report.**
2. **State audit FIRST, transcription second.** Staged records + killed-leg caches are
   untrusted until they pass each county's sole-writer path
   (`<county>/campaign_finance/make_itemized_caches.py` re-screens every record from
   scratch) + the arithmetic gates. Report kept/redone counts.
3. Contract unchanged: arithmetic-first (ARITHMETIC CLOSURE OUTRANKS GLYPH READING —
   GOTCHAS); `pct:` geometry on every row; full-page first read, tight-crop 600–1400 dpi
   escalation for disputed cells only; zero-glyph ruling; per-filing checkpoints;
   no-schedule-page vs blank-page-read recorded as DIFFERENT facts. Summit's six "source
   properties established" (AVAILABILITY.md) are binding precedent; the statewide
   17-16-6.5 line-1-vs-line-2 contribution basis (juab/wasatch finding, LEADS.md) applies.
4. **Summit grain question is OWNER-GATED — do not decide.** Keep publishing only where
   the form prints a reconciled cumulative ledger; withhold period-only sides with both
   figures named, parked in `_meta_itemized.withheld_rows`. The evidence package for a
   ruling (Wolbach 24390 / McKenna 24384 + a proposed rule) is in LEADS.md.
5. Wave agents NEVER run `scripts/build_cities_db.py` and NEVER edit root
   TODO.md/LEADS.md/HANDOFF.md/GOTCHAS.md — leads go in their reports; the coordinator
   files them.
6. **At wave close, the coordinator runs ONE federation.** It picks up work already
   STAGED and waiting in `scripts/build_cities_db.py`'s CAVEATS table (edited, unfederated:
   juab `cf-partial-structured` full-itemization text · wasatch `cf-totals-tier`
   anchor-aware text · summit `cf-totals-tier` partial-layer text · slc `cf-honest-zero`
   10-filing update) **and** the on-disk slc CF extension — ⚠ **gov.db is currently STALE
   for slc CF** (db: 8 filings/222C/162E; disk: 10/248/176) — plus juab/wasatch's new rows.
   Then `check_doc_numbers.py` (reconcile any moved doc lines — the root CLAUDE.md
   county-CF bullet will need the wave's numbers), marquee 5/5, `validate_entity.py
   --federation` 44/44.
7. Measured spend for pacing: completed legs ran ~460–590k tokens each; vision scans cost
   ~15–20k tokens/filing (born-digital nearly free). Remaining estimates: summit residue
   ~1.5–1.8M · weber ~1.2–1.6M · utah wave ~5M · cache pre-2022 + washington ~2–3M.

## Also landed 2026-08-14 (outside the wave)

- **SLC CF re-hunt (separate owner session):** the 2 IA-blocked 2003 filings recovered —
  slc now 10 of 11 filings (248C+176E, additive rebuild, validate PASS; `Dale_Lambert`
  permanent). Portal still 503, byte-identical body. **GRAMA package DRAFTED, NOT SENT**
  (`slc_city_council/campaign_finance/GRAMA_PREP_2026-08-14.md`; Series 26415 permanent
  retention + §10-3-208(10) posting duty). New host discovered: `webdme.slcgov.com`
  Laserfiche (8 public apps, no CF subtree — minutes/packets lead in LEADS.md).
- **Progress tracker (owner-facing):** `track_phaseb.py` + `PROGRESS.md` in the session
  scratchpad drove c11 sidebar bars/pulse; the loop is stopped and the scratchpad is
  session-scoped — REBUILD it fresh next session if wanted (it measures `vision/` caches +
  the two staging dirs; pulse = last-write age; ~60 lines, disposable).
- juab/wasatch/summit leads filed in LEADS.md (dated 2026-08-14 blocks), including three
  new calibration-specimen candidates (specimen-row drop class · two-digit bistable where
  arithmetic closes BOTH ways so escalation is mandatory · swapped-pages).

## Owner decisions open

- **Resume go** for summit+weber (this file is the brief). After they close: per-wave go
  for **utah 245** · **cache pre-2022 Carr era** · **washington ~90 scans**.
- **Summit period-vs-cumulative grain ruling** (evidence + proposed rule in LEADS.md).
- **SLC GRAMA send** (drafted, ready).
- **COMMIT** — everything since `34b34739a` is uncommitted (wave outputs, caveat staging,
  slc extension, doc updates).
- G9 (public flip) still parked; [GATED] retro-anchor/blind-reverify program unchanged.
