...
transcript (Murray City). Strips <c>/<timestamp> inline tags and collapses
...
HDR = ("**AUTOMATIC TRANSCRIPTION — ASR, expect word errors; not an official "
       "record.** YouTube auto-generated captions from the MURRAY CITY LIVE "
       "channel; speaker labels are absent and proper nouns (names, street/case "
       "numbers) are frequently misrecognized. The authoritative record is the "
...
def main():
    ...
    md = (f"# Murray City — {body} meeting stream, {date} (AUTOMATIC TRANSCRIPTION)\n\n"
          f"> {HDR}\n\n"
          f"- **Date:** {date}\n- **Body (inferred):** {body}\n"
          f"- **Video:** https://www.youtube.com/watch?v={vid}\n"
          f"- **Channel:** MURRAY CITY LIVE (youtube.com/channel/UC_19hfQocAIWupAD5-h6oaw)\n"
