...
def normalize_vote(tok):
    ...
    if t.startswith("recus"):
        return "Recuse"
    # §4 vote values are a CONTROLLED vocabulary (Aye|Nay|Abstain|Recuse|Absent|Excused),
    # NOT verbatim — the printed "Yes"/"No" tokens normalize to Aye/Nay (only result/
    # motion_type are city-faithful/verbatim).
    ...
    for r in rows:
        ...
        jpath.write_text(json.dumps(
            {"date": r["date"], "title": r["title"], "source_url": r["source_url"],
             "path": r["path"], "motions": motions}, indent=1), encoding="utf-8")
        n_meetings += 1
        for mo in motions:
            n_motions += 1
