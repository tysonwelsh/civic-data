#!/usr/bin/env python3
"""Magna PMN backfill — crawl a public body's cumulative notices list and parse
every notice + its attachments (GET-only, via the cumulative page=N trick).

Usage:  python3 magna_pmn_crawl.py <body_id> [<body_id> ...]
Writes work/notices_<body>.html (raw) and work/parsed_<body>.json.

Type labels on PMN are UNRELIABLE (Magna CRA draft-minutes were labeled
"Public Information Handout") — so we keep every attachment's filename + label and
let the caller detect minutes by FILENAME + CONTENT.
"""
...
def parse(html):
    ...
    for row in rows[1:]:
        ...
        atts = []
        # each attachment: an <a href="/pmn/files/ID.EXT" aria-label="Download NAME (opens...">
        # optionally followed by &nbsp;(LABEL)
