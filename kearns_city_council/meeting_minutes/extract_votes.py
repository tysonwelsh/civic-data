...
# ---------------------------------------------------------------------------
# COUNCIL fixed roster (surname -> full name), observed across the full 2017-2026
# township->city span.  Township (2017-2025): Bush(Chair,presiding/voting), Snow,
# Peterson, Schaeffer, Butterfield, and Ruby Brown (an earlier 5th seat, 2018-2021
# window — the council held 5 seats throughout; membership rotated).  City (Jan
# 2026+): Mayor Valdez(presiding/voting) + Schaeffer, Butterfield, Longtin, Colby.
# Max full-council tally = 5.
# ---------------------------------------------------------------------------
SURNAME_TO_FULL = {
    "bush": "Kelly Bush",
    "snow": "Tina Snow",
...
# Township-era per-member ROLL CALL (some 2018-2023 minutes print a full named roll
# call — "Roll was called showing the vote to be Council Member Schaeffer 'Nay,'
# Council Member Snow 'Aye,' Mayor Bush 'Aye' ..."). When the source names each
# member's vote, capturing them verbatim is faithful (NOT the tally-only case).
# Scoped strictly to an explicit roll-call block, so it never touches the 2024+
# narrative-tally motions ("vote was 5-0").
# also accept the "(carried|passed|failed) by the following vote:" lead-in — the
# 2018-23 back-catalog prints full quoted rolls under it with NO "roll call" words
# (T3.1(f) 2026-07-12: m227's 5-aye roll had 0 vote rows)
ROLLCALL_HDR = re.compile(r"[Rr]oll\b.{0,20}?call|"
                          r"(?:carried|passed|failed)\s+by\s+the\s+following\s+vote", re.I | re.S)
RC_MEMBER_VOTE = re.compile(
    r"(?:Council\s*Members?|Councilmembers?|Mayor|Vice[\s-]?Chair(?:man|person)?|"
    r"Chair(?:man|woman|person)?)\s+([A-Z][A-Za-z'\-]+)\s*[,]?\s*[\"“”']?\s*"
    r"(Aye|Yea|Nay|Abstain\w*|Absent)\b", re.I)

def parse_rollcall(region):
    """If REGION contains an explicit named roll call (>=3 members resolving to the
    roster), return {aye,nay,abstain,absent} verbatim; else None."""
    h = ROLLCALL_HDR.search(region)
    if not h:
        return None
    seg = region[h.start(): h.start() + 550]
    aye, nay, abstain, absent, seen = [], [], [], [], set()
    for vm in RC_MEMBER_VOTE.finditer(seg):
        nm = canon(vm.group(1))
        if not nm or nm in seen:
            continue
        seen.add(nm)
        act = vm.group(2).lower()
        if act.startswith(("aye", "yea")):
            aye.append(nm)
        elif act.startswith("nay"):
            nay.append(nm)
        elif act.startswith("abstain"):
            abstain.append(nm)
        elif act.startswith("absent"):
            absent.append(nm)
    if len(seen) < 3:
        return None
    return {"aye": sorted(set(aye)), "nay": sorted(set(nay)),
            "abstain": sorted(set(abstain)), "absent": sorted(set(absent))}
...
def extract_council(flat, file_body):
    ...
    for i, (astart, aend, mover, sec_known) in enumerate(anchors):
        ...
        # township-era named roll call (overrides tally-only when the source names
        # every member's vote — faithful capture, not fabrication)
        aye_named, absent_named = [], []
        names_recorded = False
        rc = parse_rollcall(region[:800])
        if rc and (rc["aye"] or rc["nay"] or rc["abstain"]):
            names_recorded = True
            aye_named = rc["aye"]
            nay_named = sorted(set(nay_named) | set(rc["nay"]))
            abstain = sorted(set(abstain) | set(rc["abstain"]))
            absent_named = rc["absent"]
            ayes = len(aye_named); nays = len(nay_named)
            unanimous = (nays == 0 and not abstain and ayes > 0)
            passed = ayes >= nays
        # verbatim-ish result
        ...
        rec = {"motion": motion_text, "body": body,
               ...
               "aye": aye_named, "nay": nay_named, "abstain": abstain,
               "absent": absent_named, "recuse": recuse,
...
def pc_outcome(votetext):
    ...
    for dm in re.finditer(r"commissioner\s+([A-Z][A-Za-z'\-]+)\s+"
                          r"(oppos\w+|abstain\w+|voting\s+(?:nay|no|against)|dissent\w+)",
                          votetext, re.I):
    ...
    with open(ROSTER, "w", encoding="utf-8", newline="") as f:
        ...
        if BODY == "Council":
            role = {"Kelly Bush": "Chair/Mayor (township)", "Tina Snow": "Vice Chair (township)",
                    "Alan Peterson": "Council (township)", "Ruby Brown": "Council (township)",
