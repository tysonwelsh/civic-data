#!/usr/bin/env python3
"""cop_ord_extract.py — text sidecars for Copperton ordinance/resolution raws.
...
Usage: python3 cop_ord_extract.py
"""
import csv
import os
import subprocess
...
SCRATCH = ("/private/tmp/claude-501/-Users-tysonwelsh-civic-data/"
           "8fb286a3-d584-4f76-9af3-7bf139a225b8/scratchpad/cop_ord_ocr")
...
def pdftotext(path):
    try:
        ...
        sys.exit("pdftotext not installed (poppler)")
...
def ocr(path, stem):
    ...
    try:
        run(["pdftoppm", "-r", "300", "-png", path, prefix], RENDER_TIMEOUT)
    except subprocess.TimeoutExpired:
