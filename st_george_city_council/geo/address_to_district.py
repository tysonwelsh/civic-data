#!/usr/bin/env python3
"""
Resolve a St. George, Utah address (or lat/long) to its City Council representation.

  *** St. George City Council is ENTIRELY AT-LARGE — there are ZERO districts. ***

The council is Mayor + 5 at-large members, all elected citywide. There is no
ward/district map, so the classic "address -> council district" lookup is
DEGENERATE: every address inside the city is represented by the same six
officials (Mayor + all 5 at-large councilmembers), and an address outside the
city has no St. George representation at all.

This tool therefore answers the only meaningful geographic question:
  "Is this address (or point) INSIDE the St. George city limits?"
...
(geo/precincts.geojson, the Washington County / UGRC VistaBallotAreas subset
that overlaps St. George) — but precinct does NOT map to a district here; it is
...
As a module:
    ...
    info = district_for_address("175 E 200 N, St. George, UT 84770")
    # -> {"in_city": True, "council": "at-large (Mayor + 5 citywide)",
    #     "precinct": "STG:12", "matched_address": "...", "lat":..., "lon":...}
...
Notes:
  ...
  - CRS: geo/*.geojson are stored in EPSG:4326 (lon/lat). The UGRC source was
    requested with outSR=4326; coords verified as Utah lon/lat (~ -113.5, 37.1),
    NOT UTM meters.
  - "district" keys are kept in the return dict for API parity with the SLC
    tool, but they are always None / "at-large" for St. George (no districts).
"""
...
def _precinct_for(lon, lat):
    ...
    for key in ("AliasName", "VistaID", "PrecinctID"):
...
def district_for_point(lon, lat):
    """lat/long -> in/out of St. George city limits (offline).
    ...
    return {
        "in_city": in_city,
        "district": None,                       # St. George has no districts
...
def _fmt(res):
    ...
    if res.get("in_city"):
        ...
        return (f"  {head}\n"
                f"    -> INSIDE St. George city limits{prec_str}\n"
                f"    -> represented by {COUNCIL}")
    return (f"  {head}\n"
            f"    -> OUTSIDE St. George city limits (no St. George representation)")


def main():
    ap = argparse.ArgumentParser(
        description="St. George address/point -> in/out of city limits "
