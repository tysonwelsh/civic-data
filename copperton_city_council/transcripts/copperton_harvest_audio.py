#!/usr/bin/env python3
"""Copperton transcripts — harvest the PMN meeting-audio (MP3) inventory.

Copperton (Town, ~800 pop; metro township 2017 -> Town 2024-05-01) is AUDIO-ONLY
(the audio-only branch of expand-city-sources, like sibling Magna / White City):
there is NO captioned video source. No YouTube channel carries Copperton meetings
(every plausible @Copperton* handle 404s; the Utah Record mirror has 0 Copperton
uploads -- see raw/_copperton_youtube_probe.txt + raw/_utah_record_channel_titles.txt).
The only durable recording medium is a per-meeting audio file attached to each Utah
Public Notice (PMN):

    Council              PMN body 5831   -> raw/_probe_5831_list.html
    Planning Commission  PMN body 1560   -> raw/_probe_1560_list.html  (MSD-staffed)
...
decided by extension, NOT the PMN category label. The meeting date comes from the
MM-DD-YYYY in the audio filename, falling back to the notice datetime.

Emits copperton_audio_inventory.csv (one row per audio file). NO audio is
downloaded -- these MP3s carry NO caption track; Whisper is a PROPOSAL only.
"""
import csv
import os
import re
...
def harvest():
    ...
    out = os.path.join(HERE, "copperton_audio_inventory.csv")
