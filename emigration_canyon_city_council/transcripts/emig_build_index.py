#!/usr/bin/env python3
"""Emigration Canyon transcripts — build index.csv (SCHEMA_SPEC.md §9 transcripts contract).

Emigration Canyon is an AUDIO-ONLY transcript entity (the audio-only branch of
expand-city-sources, like siblings Copperton / Magna / White City). There is NO
captioned video source:
  - No YouTube channel carries Emigration Canyon meetings. Every plausible
    @Emigration* handle 404s EXCEPT a personal "Noah plays piano" account
    (@emigrationcanyon) that is a name-collision decoy holding 3 non-government
    videos from 2009 -- see raw/_emig_youtube_probe.txt +
    raw/_emig_channel_videos.txt. The Utah Record mirror has 0 Emigration --
    see raw/_utah_record_channel_titles.txt.
  - Meeting video, where it exists at all, is a live-only Zoom/virtual hybrid
    (unarchived).
...
Inputs (sibling scripts in this dir):
  emig_audio_inventory.csv  (emig_harvest_audio.py) — PMN audio->date map
  emig_audio_sizes.csv      (emig_probe_sizes.py)   — HEAD liveness + bytes
...
def load_sizes():
    m = {}
    p = os.path.join(HERE, "emig_audio_sizes.csv")
    if os.path.exists(p):
...
def audio_rows(sizes):
    rows = []
    for r in csv.DictReader(open(os.path.join(HERE, "emig_audio_inventory.csv"))):
        url = r["audio_url"]
...
def main():
    ...
    print(f"  media types: {dict(Counter(r['media_type'] for r in rows))}")
