#!/usr/bin/env python3
"""Build West Valley City (Salt Lake County, UT) election_results from the COUNTY
CANONICAL SOVC. Cycles: 2019, 2021, 2023, 2025 municipal general; WVC council + mayor.
...
derives DIRECTLY from the single county canonical held at
  ...
  salt_lake_county/elections/election_results_by_contest.csv   (contest -> office/district)
The per-city raw SOVC copies were retired after verifying this build reproduces all three
CSVs BYTE-IDENTICALLY. This is now possible because the county canonical's 2026-07-19
suppression-recovery repaired the very gap (dropped un-suppressed per-precinct 2021 totals)
that originally forced this city to parse raw. Reproducible: python3 build_wvc_elections.py

WVC: 7-member council = 4 districts (1-4) + 2 at-large + separately-elected Mayor. The old
build modeled every contest single-winner (top vote-getter); that convention is preserved.
"""
# ---------------------------------------------------------------------------
...
def canon(office, district):
    if office == 'Mayor':
        return ('Mayor', '', 'West Valley City Mayor')
    if district == 'At-Large':
        return ('Council', 'At-Large', 'West Valley City Council At-Large')
    return ('Council', district, f'West Valley City Council District {district}')
...
for year, label, office, district in jobs:
    ...
    real = [(n, v) for n, v in items
            if not (n.startswith('Write-in (unresolved)') and (v or 0) == 0)]
    ...
    row.update({'year': year, 'election_type': 'municipal general', 'office': office_c,
        'district': district_c, 'contest': contest, 'n_candidates': n_candidates,
        'total_votes': total_votes, 'winner': winner, 'winner_votes': winner_votes,
        'winner_pct': wp, 'runner_up': runner_up, 'runner_up_votes': runner_up_votes,
...
writecsv(os.path.join(OUT,'west_valley_races.csv'), races, RACES_COLS)
writecsv(os.path.join(OUT,'west_valley_results_by_candidate.csv'), by_cand,
    ['year','election_type','office','district','contest','candidate','votes','pct','rank','is_winner'])
writecsv(os.path.join(OUT,'west_valley_results_by_precinct.csv'), by_precinct,
