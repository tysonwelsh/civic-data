#!/usr/bin/env python3
"""
Resolve a West Valley City address (or lat/long) to its City Council DISTRICT (1-4).

West Valley City elects a 7-member council: 4 district seats + 2 at-large seats
(plus a separately-elected Mayor). The two at-large councilmembers and the Mayor
represent the WHOLE city, so this tool only resolves the DISTRICT seat — every WVC
resident is additionally represented by both at-large members and the Mayor.

Pipeline:
  address --[Census geocoder, free]--> lat/long
  lat/long --[point-in-polygon vs precincts.geojson]--> PrecinctID (e.g. WVC003)
...
CLI:
    ...
    python3 address_to_district.py --latlon 40.6916 -111.9381
...
As a module:
    ...
    info = district_for_address("3600 S Constitution Blvd, West Valley City, UT")
    # -> {"district": "2", "precinct": "WVC0..", "matched_address": "...", "lat":..., "lon":...}

Notes:
  - Geocoding needs internet (Census API, free, no key). lat/long lookups are fully offline.
  - The district map is CURRENT (post-2022 redistricting), built from the 2023 (Districts
    1 & 3) and 2025 (Districts 2 & 4) generals. Rebuild with other years for historical
    questions: python3 build_precinct_district_map.py --years YYYY,YYYY
  - At-large seats and the Mayor are city-wide; they have no district and are not returned.
  - Precinct boundaries are WVC only; points outside WVC return district None.
"""
...
def district_for_point(lon, lat):
    """lat/long -> {district, precinct} (offline). district None if outside WVC precincts."""
...
def _fmt(res):
    ...
    if not res.get("precinct"):
        return f"  ({res.get('matched_address','')}) -> outside West Valley City precincts"
    ...
    return (f"  {head}\n    -> precinct {res['precinct']} -> Council District {d}"
            f"\n    (plus 2 city-wide At-Large councilmembers + the Mayor)")


def main():
    ap = argparse.ArgumentParser(description="West Valley City address/point -> council district (1-4)")
