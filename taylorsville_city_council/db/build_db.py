...
"""
build_db.py — normalized relational database for TAYLORSVILLE CITY's civic vote data
(prose/PDF minutes portal — the vendor-agnostic prose template, adapted for Taylorsville).

This is the WITHIN-BODY EXACT CORE. It consumes the per-body denormalized vote CSVs
(meeting_minutes/all_votes.csv [Council + RDA] + planning_commission/all_votes.csv
[PlanningCommission], both the standard 13-col schema) and produces db/taylorsville.db
...
Taylorsville minutes carry NO structured agenda/matter key, so the project key is RESOLVED
FROM PROSE — EXCEPT the Planning Commission, whose motions cite a Utah planning CASE NUMBER in
Taylorsville's `<SEQ><LETTER><YY>` form (Z=rezone/text, S=subdivision, C=conditional-use,
G=general-plan, P=permitted-use; two-letter SI/GP variants observed) — e.g. `15Z19`, `12S19`,
`29C20`, `2SI24`, cited in the minutes as `File #15Z19`. That case number is an EXACT
within-body key for the Planning Commission and is the top prose-resolution tier here.
Council motions cite ORDINANCE / RESOLUTION numbers instead and NEVER cite the PC case number
(verified: 0 of 613 council/RDA motions carry one) — so, like South Jordan's one-sided PL
bridge (and UNLIKE Millcreek), the case-number key does NOT bridge PC->Council; the cross-body
tie is reconstructed from subject + address + temporal signals in build_referrals.py.
...
Resolution tiers (recorded on every motion as app_match_method + app_confidence):
  ...
  case_no   (high)  — a `<PREFIX>-<YY>-<NNN>` planning case number cited in the motion text (exact)
...
FAILS if any row is dropped without documentation. (Millcreek has 0 duplicate/undecodable rows.)
...
_existing = sorted(glob.glob(os.path.join(HERE, "*.db")))   # reuse an existing db name; else taylorsville.db
DB = _existing[0] if _existing else os.path.join(HERE, "taylorsville.db")
...
# Utah planning CASE NUMBER — the PC's exact within-body case key. Taylorsville numbers land-use
# files as <SEQ><LETTER><YY>: a 1-3 digit sequence, a type letter, and a 2-digit year.
# Letters observed in Taylorsville PC minutes: Z (rezone / zone text), S (subdivision), C
# (conditional use), G (general plan), P (permitted use); two-letter SI and GP variants also
# occur (e.g. 2SI24, 1GP25). Cited in the minutes as "File #15Z19" / "File 5Z20". The trailing
# 2-digit year + leading multi-digit sequence keep it from colliding with Council's Ordinance /
# Resolution "YY-NN" numbers (which have no embedded type letter). Council cites NONE of these.
CASE_RE = re.compile(r'\b\d{1,3}(?:SI|GP|[ZSCGP])\d{2}\b', re.I)
...
LU_TYPES = {"land-use/zoning",                 # Council/CRA inherently-land-use motion_type
            # Planning Commission native land-use taxonomy (every one is a project application):
            "other land-use", "subdivision", "exception", "conditional use", "rezone",
            "street vacation", "zone text amendment", "general plan amendment",
...
def outcome_of(res, disp=None):
    r = (res or "").lower()
    # a withdrawn-before-vote motion has NO outcome (T3.1(l) 2026-07-12; PC 2024-07-09
    # m5 "Withdrawn (no vote)") — honestly unknown, never a default Pass
    if "withdrawn (no vote)" in r: return None
...
def stage_of(body, res, title):
    ...
    if kind_of(body) == "agency":
        ...
        if "cra" in n or "reinvest" in n: return "cra_vote"     # Millcreek Community Reinvestment Agency
        return "rda_vote"
...
CREATE TABLE motion(
  ...
  stage TEXT CHECK(stage IN ('council_vote','rda_vote','cra_vote','mba_vote','ha_vote','boa_action','other_action','pc_recommendation','pc_final_action')),
  ...
  app_match_method TEXT CHECK(app_match_method IN ('case_no','name','singleton','override') OR app_match_method IS NULL),
...
def main():
    ...
    for (src, mno), m in motions.items():
        ...
        cn = case_number(title)
        # a cited case number is inherently a land-use planning case (worthy even if the
        # native motion_type is 'Other'/'Procedural' — e.g. a plat approval labelled Procedural)
        worthy = application_worthy(body, m["motion_type"], title) or bool(cn)
        if ovkey:
            app_id = get_app(f"{body}|{ovkey.lower()}", body, ovkey, title); method, conf = "override", "high"
        elif cn:
            # exact within-body case key: every motion citing <PREFIX>-<YY>-<NNN> groups into one application
            nm = None if CODE_AMEND_RE.search(title or "") else project_name(title)
            app_id = get_app(f"{body}|case|{cn.lower()}", body, nm, title); method, conf = "case_no", "high"
        elif worthy:
            nm = None if CODE_AMEND_RE.search(title or "") else project_name(title)
            nk = name_key(nm)
